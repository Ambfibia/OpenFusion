...
#define PACKET(id) packets.emplace(id, PacketDesc(id, sizeof(s##id), #id))
#define MANUAL(id) packets.emplace(id, PacketDesc(id, sizeof(s##id), #id))
#define VAR_PACKET(id, memb, tr) packets.emplace(id, PacketDesc(id, sizeof(s##id), #id, offsetof(s##id, memb), sizeof(tr)))
...
void Packets::init() {
    ...
    VAR_PACKET(P_FE2CL_NPC_CUSTOM_ATTACK_PCs, iPCCnt, sAttackResult); // reserved; no confirmed client-side handler
    PACKET(P_FE2CL_NPC_SELF_EFFECT); // reserved; no confirmed client-side handler
    VAR_PACKET(P_FE2CL_REP_PRESENT_NPC_TYPES, iCnt, int32_t);
